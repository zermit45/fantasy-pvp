...
function quimicaAccordionHTML(q, key){
  ...
  let h=`<div class="line" style="padding:7px 4px;border-top:1px solid var(--line);margin-top:4px;cursor:pointer" onclick="toggleQuim('${key}')">`
    ...
    +`<span style="color:var(--dim);font-size:10px">(real · quem entrou em campo)</span>`
...
function quimicaGuideHTML(){
  ...
  return `<div class="modal" onclick="toggleQuimicaGuide()"><div class="box" onclick="event.stopPropagation()" style="max-height:85vh;overflow:auto">
    ...
    <p class="p" style="font-size:11.5px;line-height:1.55;margin:2px 0 6px"><b style="color:var(--chalk)">Formou o trio, ganhou:</b> escale as 3 personalidades do trio (posições diferentes) e o time leva <b style="color:#34d399">+3.5</b> na hora. Simples, sem condição.</p>
    <p class="p" style="font-size:11.5px;line-height:1.55;margin:2px 0 6px"><b style="color:var(--chalk)">Nenhum par se repete:</b> cada personalidade está em 2 trios, mas sempre com companheiros diferentes — dá pra variar bastante a montagem.</p>
    ...
    <p class="p" style="font-size:11.5px;line-height:1.55;margin:2px 0 10px">Vale só pra <b>quem entra em campo</b>: se um titular do trio não jogar, o trio não conta. Se o reserva entrar no lugar, vale a personalidade de quem entrou.</p>

    ${sec("🔺 Os 8 trios")}
    <p class="p" style="font-size:11px;color:var(--dim);margin:2px 0 6px">Forme os 3 (posições diferentes) e o time leva +3.5:</p>
    ${(Q.TRIOS||[]).map(t=>{
      ...
      return `<div style="display:flex;align-items:center;gap:10px;margin:6px 0;padding:9px;background:color-mix(in srgb,#34d399 6%,transparent);border:1px solid color-mix(in srgb,#34d399 22%,transparent);border-radius:10px">`
        ...
        +`<div style="flex:1"><div style="font-weight:700;color:var(--chalk);font-size:13px">${t.nome}</div>`
...
function superManualHTML(){
  ...
  return `<div class="modal" onclick="toggleManual()"><div class="box" onclick="event.stopPropagation()" style="max-height:85vh;overflow:auto">
    ...
    ${sec("6. Pontuação",
      ...
      p(`${b("Química:")} um bônus ${b("por time")} (não por jogador), separado da tática. Cada jogador tem uma personalidade (Maestro, Matador, Muro...); o jogo soma de uma vez ${b("todos")} os combos (personalidades que se completam) e reforços (repetir a mesma), sem importar a ordem, e adiciona ao total — até um ${b("teto de +4,5")}. Passou do teto, o excedente é cortado. ${b("Vale só pra quem entra em campo:")} na montagem você vê uma ${b("previsão")} (potencial dos titulares) e no resultado conta a química ${b("real")}, só de quem jogou — se um titular não atua, a química dele não conta, e se o reserva entra no lugar, vale a persona de quem entrou. Detalhes e a lista completa no 🧬 Guia de Química.`)+
