...
function buildHTML(){
  ...
  if(gameLocked){
    ...
    return `<div class="scorebar"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div class="tag">${esc(pp.comp)} · ⚽ EM ANDAMENTO</div><div class="userchip" onclick="${inRound?`go('round',null,'${APP.roundId}')`:"go('room')"}" style="cursor:pointer;flex-shrink:0">← voltar</div></div>
      <div class="score disp"><div class="side"><div class="fl">${flagOf(pp.home.code)}</div><div class="cd">${esc(pp.home.code)}</div><div class="nmfull">${esc(pp.home.name)}</div></div><div class="vs mono">×</div><div class="side"><div class="fl">${flagOf(pp.away.code)}</div><div class="cd">${esc(pp.away.code)}</div><div class="nmfull">${esc(pp.away.name)}</div></div></div></div>
    <div class="card">
