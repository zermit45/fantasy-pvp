...
async function loadRoom(roomId){
  APP.prepool=await loadJSON("data/"+roomId+"/prepool.json");
  try{APP.match=await loadJSON("data/"+roomId+"/match.json");}catch(e){APP.match={status:"pending"};}
...
// BOOT
// ============================================================
window.ENGINE_TACTICS = (typeof TACTICS!=="undefined")?TACTICS:(makeEngine({team_stats:{},goals_tl:[]}).TACTICS);
(async function boot(){
  try{
    const idx=await loadJSON("data/jogos.json");
    APP.jogos=idx.jogos;
  }catch(e){APP.jogos=[];}
  await tryAutoLogin();
