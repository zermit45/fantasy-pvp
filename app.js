...
async function computeRoundRanking(roundId){
  try{
    ...
    if(isConf||isBoostMode){
      ...
      let poolN=0;
      if(isBoostMode){
        const r=APP.round;        if(r&&Array.isArray(r.boost_chips)&&r.boost_chips.length)poolN=r.boost_chips.length;
...
function draftHTML(){  const s=APP.draftSeason;
...
function buildHTML(){
  ...
  return `<div class="card buildhero">
    ...
    <div class="budget"><div class="h2 disp">Seu time${helpBtn("slots")}</div><div><span class="tag">RESTANTE${helpBtn("orcamento")} </span><span class="val mono">${left}</span><span class="tag"> /110</span></div></div>
...
function place(pid){
  ...
  if(t==="BENCH"){
    ...
    if(110-spent-p.price<0){APP.warn="Orçamento estourado.";render();return;}
  }
...
function computeMedals(st){
  ...
  const add=x=>{if(x)m.push(x);};  add(tier(st.wins,[[1,"Primeira Vitória"],[3,"Vencedor"],[7,"Campeão de Sala"],[15,"Dominador"]],"🏆","wins","vitória(s)"));
...
function rulesModalHTML(){
  return `<div class="modal" onclick="toggleRules()"><div class="box" onclick="event.stopPropagation()" style="max-height:80vh;overflow:auto">
    ...
    <p class="p" style="margin:10px 0"><b style="color:var(--chalk)">Orçamento:</b> 110 moedas. Cada jogador tem um preço (calculado por qualidade técnica: valor de mercado corrigido pela idade). O banco também conta no orçamento.</p>
...
function superManualHTML(){
  ...
  return `<div class="modal" onclick="toggleManual()"><div class="box" onclick="event.stopPropagation()" style="max-height:85vh;overflow:auto">
    ...
    ${sec("3. Montar seu time",
      p(`${b("Orçamento:")} 110 moedas. Cada jogador tem um preço que reflete a qualidade dele. O banco também conta no orçamento.`)+
